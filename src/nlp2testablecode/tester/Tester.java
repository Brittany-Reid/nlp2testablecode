...
public class Tester {
	...
	public final static String FUNCTIONNAME = "snippet";
	public final static String JUNITTESTNAME = "JUnitTest";
...
//			}
	...
	private static int run(String code) {
		...
		//If testing is false, abandon
		//we shouldnt reach here but as a final measure
		if(Activator.testing == false) {
			System.err.println("Error: Testing Disabled! Enable Testing in Activator.java. This will allow arbitrary code to be executed!");
			return passed;
		}
		
		
		try {
			passed = TestProcessRunner.exec(10000, Tester.JUNITTESTNAME, className, code);
		} catch (Throwable e) {
	...
	private static MethodDeclaration constructJunit(String test) {
		...
		method.setName(Tester.JUNITTESTNAME);
